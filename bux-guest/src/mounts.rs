...
///
/// libkrun's TSI (Transparent Socket Impersonation) provides network
/// connectivity, but the guest still needs `/etc/resolv.conf` for DNS.
/// We write well-known public resolvers as a fallback when the rootfs
/// does not provide a usable resolver configuration.
pub fn setup_network() {
    let resolv = std::path::Path::new("/etc/resolv.conf");

    // If a working resolv.conf already exists (e.g. from the OCI image), leave it.
    if resolv.is_file() {
        if let Ok(content) = fs::read_to_string(resolv) {
            if content
                .lines()
    ...
    let _ = fs::write(
        resolv,
        "# Generated by bux-guest\nnameserver 8.8.8.8\nnameserver 1.1.1.1\n",
    );

    // Ensure hostname resolution works for localhost.
    ...
    // Set a hostname so tools that read it don't error.
    let hostname = std::path::Path::new("/etc/hostname");
    if !hostname.is_file() {
        let _ = fs::write(hostname, "bux\n");
    }
    let Ok(name) = std::ffi::CString::new("bux") else {
        return;
    };
    unsafe { libc::sethostname(name.as_ptr(), 3) };
}
