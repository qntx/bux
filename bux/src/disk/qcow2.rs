...
#![allow(
    ...
    reason = "binary format parser with manual bounds checks before every slice"
)]
...
pub(super) fn read_header(path: &Path) -> io::Result<QcowHeader> {
...
pub(super) fn resize(path: &Path, new_size: u64) -> io::Result<()> {
...
#[allow(dead_code, reason = "used conditionally by backing chain walker")]
pub(super) fn read_backing_file(path: &Path) -> io::Result<Option<String>> {
    let mut f = std::fs::File::open(path)?;
    let mut hdr = [0u8; 20];
...
#[allow(clippy::too_many_lines, reason = "cohesive disk flattening operation")]
pub(super) fn flatten(src: &Path, dst: &Path) -> io::Result<()> {
...
#[allow(
    ...
    reason = "tests use unwrap and let _ for clarity"
)]
mod tests {
    use super::*;
